...
# oder: PYTHONPATH=src python -m src.integrations.pidea.automation doctor
```
...
```bash
export PIDEA_CDP_HTTP_URL=http://127.0.0.1:9222
PYTHONPATH=src python -m src.integrations.pidea.tools.scan_chat_selectors
```
...
```bash
PYTHONPATH=src python -m src.integrations.pidea.tools.scan_chat_selectors --all src/integrations/pidea/selectors/cursor/1.7.17.json
```
