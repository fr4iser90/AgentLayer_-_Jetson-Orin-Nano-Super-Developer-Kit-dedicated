# Agent Layer

visit https://github.com/fr4iser90/AgentLayer_-_Jetson-Orin-Nano-Super-Developer-Kit-dedicated

**Open WebUI / custom front ends:** [docs/WEBUI_CONTRACT.md](docs/WEBUI_CONTRACT.md) (HTTP/API); [docs/WEBUI_DESIGNS.md](docs/WEBUI_DESIGNS.md) (workspace, chat badges, image page — copy or link from your UI repo).

**First-party web UI (plan, stack, English):** [docs/FRONTEND_AGENT_UI_PLAN.md](docs/FRONTEND_AGENT_UI_PLAN.md).
