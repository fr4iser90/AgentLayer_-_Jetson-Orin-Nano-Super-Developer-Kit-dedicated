...
export type WorkspaceHubId = "pets" | "family" | "media" | "home" | "work" | "other";
...
export const DEFAULT_HUBS: WorkspaceHub[] = [
  { id: "pets", label: "Pets" },
...
export type HubRules = {
  kindToHub?: Partial<Record<string, WorkspaceHubId>>;
  titleIncludes?: Partial<Record<WorkspaceHubId, string[]>>;
};

const DEFAULT_KIND_TO_HUB: Record<string, WorkspaceHubId> = {
  pets: "pets",
  photo_album: "media",
  "photo-album": "media",
  shopping_list: "home",
...
const DEFAULT_TITLE_INCLUDES: Record<WorkspaceHubId, string[]> = {
...
export function inferHubId(w: WorkspaceSummary, rules?: HubRules): WorkspaceHubId {
  ...
  for (const hub of Object.keys(byTitle) as WorkspaceHubId[]) {
...
export function groupWorkspacesByHub(
  list: WorkspaceSummary[],
  hubs: WorkspaceHub[] = DEFAULT_HUBS,
  rules?: HubRules
): Record<WorkspaceHubId, HubGroup> {
  const out = {} as Record<WorkspaceHubId, HubGroup>;
...
export function hubForSelectedId(
  grouped: Record<WorkspaceHubId, HubGroup>,
  selectedId: string | null
): WorkspaceHubId | null {
  if (!selectedId) return null;
  for (const hid of Object.keys(grouped) as WorkspaceHubId[]) {
