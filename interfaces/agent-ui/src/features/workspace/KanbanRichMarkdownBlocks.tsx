          ...
          >
            <div className="mb-2 flex items-center gap-1">
              {readOnly ? (
                ...
                <input
                  type="text"
                  className="workspace-grid-no-drag min-w-0 flex-1 rounded border border-surface-border bg-black/40 px-2 py-1 text-sm text-white"
                  value={col.title}
                  ...
                  {readOnly ? (
                    ...
                    <>
                      <input
                        type="text"
                        placeholder="Karte"
                        className="workspace-grid-no-drag mb-2 w-full rounded border border-surface-border bg-black/40 px-2 py-1 text-sm text-white"
                        value={card.title}
                        onChange={(e) => updateCardTitle(ci, card.id, e.target.value)}
                      />
                      <div className="flex flex-wrap items-center gap-2">
                        <select
                          className="workspace-grid-no-drag max-w-full flex-1 rounded border border-surface-border bg-black/40 px-1 py-0.5 text-[10px] text-neutral-200"
                          value={ci}
            ...
            {!readOnly ? (
              <button
                type="button"
                className="workspace-grid-no-drag mt-2 rounded-md border border-dashed border-white/15 py-1.5 text-xs text-surface-muted hover:border-sky-500/40 hover:text-sky-300"
                onClick={() => addCard(ci)}
              >
                + Karte
              </button>
    ...
    <section className="rounded-xl border border-surface-border bg-surface-raised/60 p-4">
      ...
      <div className="grid gap-3 lg:grid-cols-2">
        <div>
          ...
          <textarea
            className="workspace-grid-no-drag min-h-[220px] w-full resize-y rounded-lg border border-surface-border bg-black/35 px-3 py-2 font-mono text-sm text-neutral-100 outline-none focus:border-sky-500/50"
