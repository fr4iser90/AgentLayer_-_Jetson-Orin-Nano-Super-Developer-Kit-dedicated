...
import type { HubGroup, WorkspaceHub, WorkspaceHubId } from "./workspaceHubNav";

const LS_FAV_KEY = "workspace_nav_favorites_v1";
...
export function WorkspaceHubNavigator(props: {
  hubs: WorkspaceHub[];
  grouped: Record<WorkspaceHubId, HubGroup>;
  activeHubId: WorkspaceHubId;
  setActiveHubId: (id: WorkspaceHubId) => void;
  ...
  const listEntries = useMemo(() => {
    const mk = (w: WorkspaceSummary): Entry => ({
      id: w.id,
    ...
    <div className="rounded-xl border border-surface-border bg-surface-raised/50 p-3 md:p-4" onKeyDown={onKeyDown}>
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="min-w-[220px] flex-1 lg:max-w-sm">
          <input
            className="workspace-grid-no-drag w-full rounded-lg border border-surface-border bg-black/30 px-3 py-2 text-sm text-white outline-none focus:border-sky-500/50"
      ...
      <div className="mt-4">
        ...
        >
          {listEntries.length === 0 ? (
            <p className="px-3 py-8 text-center text-sm text-surface-muted">No workspaces in this hub.</p>
          ) : (
                ...
                return (
                  <li key={e.id} className="flex items-stretch gap-1">
                    <button
                      ...
                      onClick={() => onSelectWorkspace(e.id)}
                    >
